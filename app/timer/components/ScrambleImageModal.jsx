    ...
    const init = async () => {
      ...
      try {
        ...
        if (use2D) {
          ...
          const display = new ScrambleDisplay({ event: displayEvent, visualization: '2D', checkered: true, alg: scramble });
          ...
          const player = new TwistyPlayer({ puzzle, alg: scramble, hint: 'none', controlPanel: 'none', background: 'none' });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-[#0f1117] border-[#2a2f3a] max-w-lg w-[90vw]">
        ...
        {scramble && <ScrambleModalInner scramble={scramble} eventId={eventId} />}
