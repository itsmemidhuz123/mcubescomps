...
export default function ScrambleVisualization({ scramble, eventId, height = '200px' }) {
  ...
  useEffect(() => {
    ...
    const puzzleType = PUZZLE_MAP[eventId];
    if (!puzzleType) {
      console.warn(`Unsupported puzzle type for event: ${eventId}, defaulting to 3x3x3`);
    }
    player.setAttribute('puzzle', puzzleType || '3x3x3');
    ...
    player.setAttribute('show-hint', 'false');
    player.setAttribute('show-steps', 'false');
    player.setAttribute('show-timestamps', 'false');
