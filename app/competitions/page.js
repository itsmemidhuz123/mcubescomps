...
function CompetitionsPage() {
    ...
    async function fetchCompetitions() {
        try {
            ...
            let compsData = snapshot.docs.map(doc => {
                const data = doc.data();
                const now = new Date();
                const startRaw = data.startDate || data.competitionStartDate;
                const endRaw = data.endDate || data.competitionEndDate;
            ...
            if (!isAdmin) {
                compsData = compsData.filter(comp => {
                    const published =
                        typeof comp.isPublished === 'boolean' ? comp.isPublished :
                            typeof comp.published === 'boolean' ? comp.published :
...
function CompetitionCard({ comp, router, isPast = false }) {
    ...
    const getRegStatus = () => {
        const now = new Date();
        const regOpenRaw = comp.registrationOpenDate || comp.registrationStartDate;
        const regCloseRaw = comp.registrationCloseDate || comp.registrationEndDate;
        const regOpen = regOpenRaw ? new Date(regOpenRaw) : null;
        const regClose = regCloseRaw ? new Date(regCloseRaw) : null;
