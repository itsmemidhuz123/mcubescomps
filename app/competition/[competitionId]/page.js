...
import { ArrowLeft, Calendar, Trophy, DollarSign, Play, Clock, Users, AlertCircle, Lock, Info, Timer, Tag, CheckCircle, XCircle, Loader2, Layers, ShieldAlert, Eye } from 'lucide-react';
...
import { getCurrentRound, getNextRound, getRoundStatus, getUserPosition, isQualified } from '@/lib/competitionLogic';
...
function CompetitionDetail() {
    ...
    // Event status state for tournament mode
                        ...
                        <CardContent className="py-6">
                            <div className="space-y-4">
                                ...
                                {(registration.events?.length > 1) ? (
                                    ...
                                    <>
                                        <p className="text-zinc-700 dark:text-zinc-300 text-center">
                                            You are registered for {registration.events?.length || 0} event(s):
                                        </p>
                                        ...
                                        {/* Single event - check status and show appropriate button */}
                                        {registration.events?.length === 1 && (
                                            <div className="text-center pt-4">
                                                {(() => {
                                                    const eventId = registration.events[0];
                                                    const status = eventStatuses[eventId] || {};
                                                    const roundStatus = status.roundStatus;
                                                    const isEventEnded = roundStatus === 'completed' || 
                                                                        roundStatus === 'verification' || 
