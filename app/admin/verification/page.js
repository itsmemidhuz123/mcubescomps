...
import { ArrowLeft, Search, Shield, ShieldCheck, ShieldAlert, Users, AlertTriangle, RefreshCw, Loader2, CheckCircle, XCircle } from 'lucide-react';
...
export default function VerificationCenterPage() {
    ...
    const filteredUsers = users.filter(u => {
        const userStatus = u.verificationStatus || 'UNVERIFIED';
    ...
    async function handleAction() {
        ...
        try {
            ...
            } else if (actionType === 'manual_reject') {
                await fetch('/api/verification/admin', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
            ...
            <div className="container mx-auto px-4 max-w-7xl">
                                                    ...
                                                    <TableCell className="text-right">
                                                        <div className="flex justify-end gap-2">
                                                            <Button
                                                                ...
                                                                onClick={() => openActionDialog(u, 'manual_verify')}
                                                                disabled={u.verificationStatus === 'VERIFIED'}
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={() => openActionDialog(u, 'manual_reject')}
                                                                disabled={u.verificationStatus === 'REJECTED'}
                                                                title="Manually reject user"
                                                            >
                                                            ...
                                                            <Button
                                                                size="sm"
                                                                variant="outline"
                                                                onClick={() => openActionDialog(u, 'reset')}
                                                                disabled={processing}
                                                            >
                                                                Reset
                                                            </Button>
                ...
                <Dialog open={showActionDialog} onOpenChange={setShowActionDialog}>
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>
                                {actionType === 'manual_verify' && 'Manually Verify User'}
                                {actionType === 'manual_reject' && 'Manually Reject User'}
                                {actionType === 'reset' && 'Reset Verification Status'}
                            </DialogTitle>
                            <DialogDescription>
                                {actionType === 'manual_verify' && `This will manually verify ${selectedUser?.displayName || selectedUser?.username || 'this user'}. They will be marked as verified without going through DIDIT.`}
                                {actionType === 'manual_reject' && `This will manually reject ${selectedUser?.displayName || selectedUser?.username || 'this user'}.`}
                                {actionType === 'reset' && `This will reset ${selectedUser?.displayName || selectedUser?.username || 'this user'}'s verification status. They will need to verify again.`}
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        <DialogFooter>
                            ...
                            <Button
                                variant={actionType === 'manual_verify' ? 'default' : 'destructive'}
                            ...
                            >
                                {processing ? (
                                    ...
                                    <>
                                        {actionType === 'manual_verify' && 'Verify User'}
                                        {actionType === 'manual_reject' && 'Reject User'}
                                        {actionType === 'reset' && 'Reset Verification'}
                                    </>
                                )}
                            </Button>
                        </DialogFooter>
