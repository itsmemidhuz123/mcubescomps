...
export default function TournamentManagementPage() {
    ...
    const [selectedEventForRounds, setSelectedEventForRounds] = useState(null);
    ...
    async function fetchTournamentData() {
        ...
        try {
            ...
            // Set default selected event for rounds config
            if (compData.events && compData.events.length > 0) {
                setSelectedEventForRounds(compData.events[0]);
            }
    ...
    // Event Rounds Management Functions
    function addRoundForEvent(eventId) {
        if (!competition) return;
        
        const eventRounds = competition.eventRounds?.[eventId] || [];
        const nextRoundNumber = eventRounds.length > 0 
            ? Math.max(...eventRounds.map(r => r.roundNumber)) + 1 
            : 1;
        ...
        setCompetition(prev => ({
            ...
            eventRounds: {
                ...prev.eventRounds,
                [eventId]: [...eventRounds, newRound]
            }
        }));
    }

    function updateRoundForEvent(eventId, roundNumber, field, value) {
        if (!competition) return;
        
        const eventRounds = competition.eventRounds?.[eventId] || [];
        const updatedRounds = eventRounds.map(round => 
        ...
        setCompetition(prev => ({
            ...
            eventRounds: {
                ...prev.eventRounds,
                [eventId]: updatedRounds
            }
        }));
    }

    function removeRoundForEvent(eventId, roundNumber) {
        if (!competition) return;
        
        const eventRounds = competition.eventRounds?.[eventId] || [];
        const filteredRounds = eventRounds.filter(r => r.roundNumber !== roundNumber);
        
        setCompetition(prev => ({
            ...prev,
            eventRounds: {
                ...prev.eventRounds,
                [eventId]: filteredRounds
            }
        }));
    }

    function setEventCurrentRound(eventId, roundNumber) {
        setCompetition(prev => ({
            ...prev,
            eventCurrentRound: {
                ...prev.eventCurrentRound,
                [eventId]: roundNumber
            }
        }));
    }

    async function saveEventRounds() {
        if (!competition) return;
        
        setSavingRounds(true);
        try {
            await updateDoc(doc(db, 'competitions', competition.id), {
                eventRounds: competition.eventRounds,
                eventCurrentRound: competition.eventCurrentRound
            });
            alert('Event rounds saved successfully!');
        } catch (error) {
            console.error('Error saving event rounds:', error);
    ...
    function resetToSharedRounds(eventId) {
        if (!confirm('Are you sure? This will remove event-specific rounds.')) return;
        
        setCompetition(prev => {
            const newEventRounds = { ...prev.eventRounds };
            delete newEventRounds[eventId];
            
            const newEventCurrentRound = { ...prev.eventCurrentRound };
            delete newEventCurrentRound[eventId];
            
            return {
                ...prev,
                eventRounds: newEventRounds,
                eventCurrentRound: newEventCurrentRound
            };
        });
    }
            ...
            <div className="container mx-auto p-6 space-y-6">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                    <TabsList className="grid w-full grid-cols-6 lg:w-[900px]">
                        ...
                        <TabsTrigger value="eventRounds">Event Rounds</TabsTrigger>
                    ...
                    {/* Event-Specific Rounds Configuration */}
                    <TabsContent value="eventRounds" className="space-y-4">
                        <Card>
                            <CardHeader>
                                <CardTitle>Configure Rounds per Event</CardTitle>
                                <CardDescription>
                                    Set different round progressions for each event. If not configured, all events share the same rounds.
                                </CardDescription>
                            </CardHeader>
                            <CardContent>
                                {/* Event Selector */}
                                <div className="mb-6">
                                    ...
                                    <div className="flex gap-2 flex-wrap">
                                        {competition.events?.map(eventId => (
                                            <Button
                                                key={eventId}
                                                variant={selectedEventForRounds === eventId ? 'default' : 'outline'}
                                                onClick={() => setSelectedEventForRounds(eventId)}
                                                className="gap-2"
                                            >
                                ...
                                {selectedEventForRounds && (
                                    <div className="space-y-6">
                                        {/* Current Rounds for Selected Event */}
                                        <div className="border rounded-lg p-4">
                                            <div className="flex items-center justify-between mb-4">
                                                <h3 className="font-semibold">
                                                    Rounds for {getEventName(selectedEventForRounds)}
                                                </h3>
                                                <Button
                                                    onClick={() => addRoundForEvent(selectedEventForRounds)}
                                                    size="sm"
                                                    className="gap-1"
                                                >
                                                    <Layers className="h-4 w-4" />
                                            ...
                                            {(competition.eventRounds?.[selectedEventForRounds] || []).length === 0 ? (
                                                ...
                                                <div className="space-y-3">
                                                    {competition.eventRounds[selectedEventForRounds].map((round, index) => (
                                                            ...
                                                            <div className="flex items-start gap-4">
                                                                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                                                    <div>
                                                                        ...
                                                                        <Input
                                                                            value={round.name || ''}
                                                                            onChange={(e) => updateRoundForEvent(selectedEventForRounds, round.roundNumber, 'name', e.target.value)}
                                                                            placeholder={`Round ${round.roundNumber}`}
                                                                        />
                                                                    </div>
                                                                    <div>
                                                                        <Label className="text-xs">Qualify Type</Label>
                                                                        <select
                                                                            className="w-full p-2 border rounded"
                                                                            value={round.qualifyType || 'count'}
                                                                            onChange={(e) => updateRoundForEvent(selectedEventForRounds, round.roundNumber, 'qualifyType', e.target.value)}
                                                                        >
                                                                    ...
                                                                    <div>
                                                                        <Label className="text-xs">Qualify Value</Label>
                                                                        <Input
                                                                            type="number"
                                                                            value={round.qualifyValue || 50}
                                                                            onChange={(e) => updateRoundForEvent(selectedEventForRounds, round.roundNumber, 'qualifyValue', parseInt(e.target.value))}
                                                                        />
                                                                    </div>
                                                                    <div>
                                                                        ...
                                                                        <select
                                                                            ...
                                                                            onChange={(e) => updateRoundForEvent(selectedEventForRounds, round.roundNumber, 'status', e.target.value)}
                                                                        >
                                                                ...
                                                                <div className="flex flex-col gap-2">
                                                                    <label className="flex items-center gap-2 text-sm">
                                                                        <input
                                                                            ...
                                                                            onChange={(e) => updateRoundForEvent(selectedEventForRounds, round.roundNumber, 'isFinal', e.target.checked)}
                                                                        />
                                                                        Final Round
                                                                    </label>
                                                                    <Button
                                                                        variant="ghost"
                                                                        size="sm"
                                                                        onClick={() => removeRoundForEvent(selectedEventForRounds, round.roundNumber)}
                                        ...
                                        {/* Set Current Round */}
                                        <div className="border rounded-lg p-4">
                                            <h3 className="font-semibold mb-4">Current Round</h3>
                                            <div className="flex items-center gap-4">
                                                <select
                                                    className="p-2 border rounded"
                                                    value={competition.eventCurrentRound?.[selectedEventForRounds] || 1}
                                                    onChange={(e) => setEventCurrentRound(selectedEventForRounds, parseInt(e.target.value))}
                                                >
                                                    {(competition.eventRounds?.[selectedEventForRounds] || []).map(round => (
                                                        <option key={round.roundNumber} value={round.roundNumber}>
                                                            {round.name || `Round ${round.roundNumber}`}
                                                        </option>
                                                    ))}
                                                </select>
                                                <Button onClick={() => saveEventRounds()} disabled={savingRounds}>
                                                    {savingRounds ? 'Saving...' : 'Save Changes'}
                                                </Button>
                                            </div>
                                        </div>

                                        {/* Reset to Shared Rounds */}
                                        {(competition.eventRounds?.[selectedEventForRounds] || []).length > 0 && (
                                            <div className="border border-red-200 rounded-lg p-4 bg-red-50">
                                                <h3 className="font-semibold text-red-800 mb-2">Reset to Shared Rounds</h3>
                                                <p className="text-sm text-red-600 mb-3">
                                                    This will remove event-specific rounds and use the shared rounds configuration.
                                                </p>
                                                <Button
                                                    variant="outline"
                                                    onClick={() => resetToSharedRounds(selectedEventForRounds)}
