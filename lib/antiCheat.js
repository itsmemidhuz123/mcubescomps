...
export class AntiCheatDetector {
  ...
  activate() {
    ...
    // Check mobile status once on activation
    this.isMobile = this.isMobileDevice();
    
    this.setupListeners();
    
    // Only run advanced detection on desktop
    if (!this.isMobile) {
  ...
  setupListeners() {
    // Tab Switch / Visibility Detection (Enabled on Mobile)
    ...
    // Window Blur Detection (Disabled on Mobile)
    if (!this.isMobile) {
      const blurHandler = () => {
        if (this.isActive) {
    ...
    // Right Click Protection (Disabled on Mobile to allow long press)
    if (!this.isMobile) {
    ...
    // Keyboard Shortcut Detection (DevTools) (Disabled on Mobile)
    if (!this.isMobile) {
    ...
    // Page Unload Detection (Refresh/Close) (Enabled on Mobile)
